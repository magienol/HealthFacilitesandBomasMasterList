...
const Facilities = () => {
  ...
  // Filter counties and county codes based on selected state
  const counties = filters.state ? 
    [...new Set(facilitiesData.filter(facility => facility.state === filters.state).map(facility => facility.county))] 
    : [...new Set(facilitiesData.map(facility => facility.county))];

  const countyCodes = filters.county ? 
    [...new Set(facilitiesData.filter(facility => facility.county === filters.county).map(facility => facility.countycode))]
    : [...new Set(facilitiesData.map(facility => facility.countycode))];

  // Filter payams based on selected county
  const payams = filters.county ? 
    [...new Set(facilitiesData.filter(facility => facility.county === filters.county).map(facility => facility.payam))]
    : [...new Set(facilitiesData.map(facility => facility.payam))];
  ...
  const filteredFacilities = facilitiesData.filter(facility => {
    ...
    return (
      (facility.name.toLowerCase().includes(lowerSearchTerm) ||
        ...
        (facility.countycode && facility.countycode.toLowerCase().includes(lowerSearchTerm)) || // Check for countyCode
      ...
      (filters.countyCode ? facility.countyCode && facility.countyCode === filters.countyCode : true) && // Check for countyCode
  ...
  // CSV Export Function
  const exportCSV = () => {
    const header = ['Serial No', 'Name', 'Facility ID', 'Type', 'Ownership', 'Location', 'Date of Establishment', 'Operating 24 Hours', 'County Code'];
  ...
  const exportPDF = () => {
    ...
    // Using autoTable to generate a table in the PDF
    doc.autoTable({
      head: [['Serial No', 'Name', 'Facility ID', 'Type', 'Ownership', 'Location', 'Date of Establishment', 'Operating 24 Hours', 'County Code']],
      ...
      <div className='filters-container'>
        ...
        <FormControl fullWidth margin='normal'>
          ...
          <Select
            ...
            onChange={handleFilterChange}
            label='County'
            disabled={!filters.state} // Disable county selection until a state is chosen
          >
            <MenuItem value=''>All</MenuItem>
        ...
        <FormControl fullWidth margin='normal'>
          <InputLabel>County Code</InputLabel>
          <Select
            name='countycode'
            value={filters.countycode}
            onChange={handleFilterChange}
            label='County Code'
            disabled={!filters.county} // Disable countycode selection until a county is chosen
          >
            <MenuItem value=''>All</MenuItem>
            {countyCodes.map((countycode, index) => (
              <MenuItem key={index} value={countycode}>{countycode}</MenuItem>
            ))}
          </Select>
        </FormControl>
        ...
        <FormControl fullWidth margin='normal'>
          ...
          <Select
            ...
            onChange={handleFilterChange}
            label='Payam'
            disabled={!filters.county} // Disable payam selection until a county is chosen
          >
            <MenuItem value=''>All</MenuItem>
        ...
        <Table>
          ...
          <TableBody>
            {paginatedFacilities.map((facility, index) => (
              <TableRow key={index} onClick={() => handleRowClick(facility)}>
                <TableCell>{(currentPage - 1) * rowsPerPage + index + 1}</TableCell>
                <TableCell>{facility.state}</TableCell>
                <TableCell>{facility.county}</TableCell>
                <TableCell>{facility.countyCode}</TableCell>
